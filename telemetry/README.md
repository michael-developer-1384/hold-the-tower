...
`hodl_candle_closed` is one event per Opening-Bell session (next bell or run resolution, not spawn-complete): `hodl_open` / `hodl_high` / `hodl_low` / `hodl_close` / `hodl_min` / `price_change` / `spawn_pressure_total` / `advance_pressure_total` / `damage_recovery_total` / `kill_gain_total` / `core_loss_total` / `realized_gain` / `realized_loss` / `kills` / `core_damage_this_wave`. Open is the HODL Price at Opening Bell. Close equals the next session's open when a bell rolls over. The price is derived from directional combat flows, not combat state. 10 Hz samples are not persisted. Summaries also include `hodl_candles`.
