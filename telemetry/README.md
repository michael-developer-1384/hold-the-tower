...
`hodl_candle_closed` is one event per combat wave (spawn-complete freeze): `hodl_open` / `hodl_high` / `hodl_low` / `hodl_close` / `hodl_min` / `core_damage_this_wave`. Open is the first sampled active-threat index, not the Opening Bell value. The index is derived presentation, not combat state. 10 Hz samples are not persisted. Summaries also include `hodl_candles`.
