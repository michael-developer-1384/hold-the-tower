...
var _stat_rows: Dictionary = {} # sid -> ResearchStatRow
var _research_page: Control
...
func _ready() -> void:
	...
	_lv_rp_label = UiStyleScript.make_lv_rp_badge(
...
func _build_research(root: Control) -> Control:
	...
	_hdr_current = UiStyleScript.make_flat_label("", 14)
	hcol.add_child(_hdr_current)
	_hdr_draft = UiStyleScript.make_flat_label("", 14)
	hcol.add_child(_hdr_draft)
	_hdr_change = UiStyleScript.make_flat_label("", 15)
	...
	_reset_btn = UiStyleScript.make_compact_button("RESET DRAFT", 160, 44)
	_reset_btn.pressed.connect(_reset_draft)
	footer.add_child(_reset_btn)
	_apply_btn = UiStyleScript.make_compact_button("APPLY", 220, 44)
...
func _build_saved_configs(body: Control) -> void:
	body.add_child(UiStyleScript.make_flat_label("SAVED CONFIGURATIONS", 16))
	body.add_child(UiStyleScript.make_flat_label(
		"Optional named saves. Research works without them.", 12, true
	))
	...
	_bp_name_edit.custom_minimum_size = Vector2(180, 36)
	save_row.add_child(_bp_name_edit)
	var save_btn := UiStyleScript.make_compact_button("SAVE CURRENT", 140, 36)
...
func _refresh_research_ui() -> void:
	...
	_lv_rp_label.text = "LV %d   RP %d" % [level, rp]
	...
	_hdr_rp.text = "RESEARCH POINTS  %d available" % rp
	...
	var can_apply := delta != 0 and (delta <= 0 or rp >= delta)
...
func _refresh_blueprint_list() -> void:
	...
	for bp in ProfileManager.get_tower_blueprints(_tower_id):
		...
		var load_b := UiStyleScript.make_compact_button("LOAD", 70, 30)
		...
		var over := UiStyleScript.make_compact_button("SAVE OVER", 100, 30)
		...
		row.add_child(over)
		var del := UiStyleScript.make_compact_button("…", 40, 30)
